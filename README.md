...
| <a name="input_tags"></a> [tags](#input\_tags) | A mapping of tags to assign to the logs bucket. Please note that tags with a conflicting key will not override the original tag. | `map(string)` | <pre>{<br>  "Automation": "Terraform"<br>}</pre> | no |
